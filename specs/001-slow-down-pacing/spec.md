...
- Q: When the user sends a prompt during a pause, does the agent respond immediately or wait for the pause to complete? → A: Immediately — user input is never delayed by a pause (the pending Stop hook is aborted). The pause *window* itself continues on the wall clock: any remaining pause applies at the next Stop boundary until the window closes, and the cycle never resets on input (deterministic wall-clock semantics per FR-011; the aborted hook persists nothing, per FR-010a).
